...
//				}else{
					for (var i = 0, file; file = files[i]; i++) {
						...
						}else if(file.type != "image/jpeg" && file.type !="image/png" && file.type !="image/jpg" && file.type !="image/bmp") {
							alert('您这个"'+ file.name +'"格式格式文件');
						}else{
							// 在这里需要判断当前所有文件中
							arrFiles.push(file);	
						}
				...
				var params = {
					...
					onSuccess: function(file, response) {
						...
						if($("#uploadTailor_"+file.index).length>0){
							var lgnsize = $("#uploadTailor_"+file.index).attr('tailor');
							alert(eval(response));
							alert(lgnsize);
